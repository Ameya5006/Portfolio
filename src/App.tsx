...
const supportingExperience = [
  ['JUL 2025 — MAY 2026', 'Sub Head · Operations', 'Student Council · events, resources and the three-day Uphoria fest'],
    ...
    <main>
      ...
      <section className="journey-section wrap section-space" id="journey">
        ...
        <div className="journey-layout"><div><div className="timeline">{experience.map(({ period, org, title, description, icon: Icon }) => <article key={title}><div className="timeline-icon"><Icon size={19} /></div><span className="mono">{period} / {org}</span><h3>{title}</h3><p>{description}</p></article>)}</div>
          <article className="current-role"><span className="current-role-icon"><Workflow size={19} /></span><div><span className="mono">AUG 2026 — PRESENT / CURRENT CAMPUS ROLE</span><h3>Placement Committee Member</h3><p>Career Services Center · Bennett University</p><small>Supporting placement coordination while continuing to build technical projects.</small></div></article>
          <details className="supporting-experience"><summary>Leadership & operations archive <span>05 more roles +</span></summary><div>{supportingExperience.map(([period, role, org]) => <article key={`${period}-${role}`}><span className="mono">{period}</span><div><h4>{role}</h4><p>{org}</p></div></article>)}</div><External href={linkedin}>Full timeline on LinkedIn</External></details>
